...
class MessageLazyBuilders {
  ...
  public function buildMessages(ThreadInterface $entity, $view_mode) {
    ...
    $account = $this->currentUser->hasPermission('read all private convesation') ? NULL : $this->currentUser;
    $load_deleted = $this->currentUser->hasPermission('read all private conversation') ? TRUE : FALSE;
    $pager_id = 0;

    $messages = $this->entityManager->getStorage('ptalk_thread')->loadThreadMessages($entity, $account, $load_deleted, $messages_per_page, 0);
    if ($messages) {
