...
class ThreadManager implements ThreadManagerInterface {
  ...
  public function createIndex($thread) {
    ...
    $participants = explode(',', rtrim($thread->participants->value, ','));
  ...
  public function countMessages($thread, $account_id = NULL, $count_deleted = FALSE) {
    ...
    if (!isset($count[$account_id])) {
      ...
      $count[$account_id] = $count_query->execute()->fetchField();
    }
   ...
   * {@inheritdoc}
   */
  public function getNewMessages($thread, $account = NULL) {
    if (is_null($account)) {
    ...
    $count = $count_query->execute()->fetchCol();
  ...
  public function increaseCounts($message, $count = 1) {
    ...
    $pids = explode(',', $message->recipients);
    $this->database->update('ptalk_thread_index')
