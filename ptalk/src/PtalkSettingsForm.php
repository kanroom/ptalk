...
/**
 * Configure ptalk settings for this site.
 */
class PtalkSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['ptalk_listing'] = array(
      '#type' => 'details',
      '#title' => $this->t('Message lists'),
      '#weight' => 0,
      '#group' => 'settings',
    );

    $form['ptalk_listing']['ptalk_messages_per_page'] = array(
      '#type' => 'select',
      '#title' => $this->t('Number of messages on thread pages'),
    ...
    $form['display_settings']['ptalk_display_loginmessage'] = array(
      '#type' => 'checkbox',
    ...
    $form['display_settings']['ptalk_display_disabled_message'] = array(
      '#type' => 'checkbox',
    ...
    $form['display_settings']['ptalk_default_redirect'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Default redirection URL'),
      '#default_value' => $config->get('ptalk_default_redirect'),
      '#description' => $this->t('Defines to which page users are sent after sending a new message. &lt;front&gt; can be used for the defined front page and &lt;new-message&gt; to redirect to the new message. Leave empty to disable the redirect.'),
    );

    $form['display_settings']['ptalk_default_redirect_reply'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Redirect replies too'),
      '#default_value' => $config->get('ptalk_default_redirect_reply'),
      '#description' => $this->t('Also redirect to the defined page when replying.'),
    );

    $form['display_settings']['ptalk_display_preview_button'] = array(
      '#type' => 'checkbox',
    ...
    $form['links']['ptalk_display_link_self'] = array(
      '#type' => 'checkbox',
    ...
    $form['links']['ptalk_display_profile_links'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Display link on profile pages.'),
      '#description' => $this->t('If this setting is enabled, a link to send a private message will be displayed.'),
      '#default_value' => $config->get('ptalk_display_profile_links'),
    );

    $node_types = node_type_get_names();
    $form['links']['ptalk_link_node_types'] = array(
      '#type' => 'checkboxes',
    ...
    $form['links']['ptalk_display_on_thread'] = array(
      '#type' => 'checkbox',
    ...
    $form['links']['ptalk_display_on_messages'] = array(
      '#type' => 'checkbox',
    ...
    $form['flush_deleted'] = array(
      '#type' => 'details',
      '#title' => $this->t('Deleted messages'),
      '#description' => $this->t('By default, deleted messages are only hidden from the user but still stored in the database. These settings control if and when messages should be removed.'),
      '#weight' => 3,
      '#group' => 'settings',
    );

    $form['flush_deleted']['ptalk_flush_enabled'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Flush deleted messages'),
      '#default_value' => $config->get('ptalk_flush_enabled'),
      '#description' => $this->t('Enable the flushing of deleted messages. Requires that cron is enabled'),
    );

    $form['flush_deleted']['ptalk_flush_days'] = array(
      '#type' => 'select',
      '#title' => $this->t('Flush messages after they have been deleted for more days than'),
      '#default_value' => $config->get('ptalk_flush_days'),
      '#options' => array(0, 1, 2, 5, 10, 30, 100),
    );

    $form['flush_deleted']['ptalk_flush_max'] = array(
      '#type' => 'select',
      '#title' => $this->t('Maximum number of messages to flush per cron run'),
      '#default_value' => $config->get('ptalk_flush_max'),
      '#options' => array(50, 100, 200, 500, 1000),
    );

    // Message status settings
    $form['ptalk_status'] = array(
      '#type' => 'details',
    ...
    $form['ptalk_status']['ptalk_status_message_status'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Private message status'),
      '#default_value' => $config->get('ptalk_status_message_status'),
      '#description' => $this->t('Configure if status of the message will be displayed on the thread page.'),
    );

    $form['ptalk_status']['ptalk_status_message_status_limit_recipients'] = [
      '#type' => 'select',
      '#title' => t('Amount of the recipients which will be outputed in the status string.'),
      '#default_value' => $config->get('ptalk_status_message_status_limit_recipients', 3),
      ...
      '#states' => [
        'visible' => [
          "input[name='ptalk_status_message_status']" => array('checked' => TRUE),
        ]
      ]
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->config('ptalk.settings')
      ...
      ->set('ptalk_default_redirect', $form_state->getValue('ptalk_default_redirect'))
      ->set('ptalk_default_redirect_reply', $form_state->getValue('ptalk_default_redirect_reply'))
      ...
      ->set('ptalk_status_message_status', $form_state->getValue('ptalk_status_message_status'))
      ->set('ptalk_status_message_status_limit_recipients', $form_state->getValue('ptalk_status_message_status_limit_recipients'))
      ->save();
  }
